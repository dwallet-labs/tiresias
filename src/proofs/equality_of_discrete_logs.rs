...
mod tests {
    ...
    fn invalid_proof_fails_verification() {
        ...
        /* First generate an zeroed-out proof and make sure it fails */
        ...
        /* Now generate a ONEd-out  proof and make sure it fails */
        let invalid_proof = ProofOfEqualityOfDiscreteLogs {
            a: N2::ONE,
            b: N2::ONE,
            g_hat: N2::ONE,
        ...
        /* Now generate a random proof and make sure it fails */
        ...
        invalid_proof.a = U4096::ZERO;
        assert!(invalid_proof.verify(&N, &G, &H).is_err());
        invalid_proof.a = U4096::ONE;
        assert!(invalid_proof.verify(&N, &G, &H).is_err());
        invalid_proof.a = a;
        assert!(invalid_proof.verify(&N, &G, &H).is_err());

        invalid_proof.b = U4096::ZERO;
        assert!(invalid_proof.verify(&N, &G, &H).is_err());
        invalid_proof.b = U4096::ONE;
        assert!(invalid_proof.verify(&N, &G, &H).is_err());
        invalid_proof.b = b;
        assert!(invalid_proof.verify(&N, &G, &H).is_err());
